...
def run_retrieval(name, dob, mobile):
    ...
    session = make_uidai_session(retries=0, log=log)
    ...
    for full_name in candidate_names:
        if technical_diff:
            break
        log(f"🔍 [RETRIEVAL] Trying name payload: '{full_name}'...")
        ...
        while attempt <= max_captcha_retries and captcha_attempts < max_captcha_attempts:
            captcha_attempts += 1
            try:
                log(f"🌐 Fetching UIDAI captcha (try {captcha_attempts}/{max_captcha_attempts})...")
                res_cap = session.post(CAPTCHA_URL, json=captcha_payload, headers=headers, timeout=20)
                ...
                log("✅ Captcha image received from UIDAI")

                img_bytes = base64.b64decode(img_b64)
                if attempt >= 3:
                    log(f"🔑 MANUAL CAPTCHA REQUIRED | {img_b64}")
                    captcha_val = sys.stdin.readline().strip()
                    if not captcha_val:
                        raise Exception("No manual captcha entered.")
                else:
                    log("🧠 Solving captcha with OCR...")
                    res = get_ocr().classification(img_bytes)
                    captcha_val = str(res or '').strip()
                    captcha_val = re.sub(r'[^a-zA-Z0-9]', '', captcha_val)
                    
                    if len(captcha_val) != 6:
                        log(f"⚠️ [OCR] Rejected noisy read '{captcha_val}' (Length {len(captcha_val)} != 6). Fetching new captcha...")
                        attempt += 1
                        continue
                    
                log(f"Decoded Captcha: {captcha_val}")
                ...
                log("📤 Submitting EID search to UIDAI...")
                res_otp = session.post(RETRIEVE_URL, json=details_payload, headers=headers, timeout=25)
                try:
                ...
                log(f"⚠️ NETWORK ERROR: UIDAI timeout for '{full_name}' ({ex})")
                last_server_msg = "NETWORK ERROR ||| UIDAI portal timeout. Server slow or blocked. Try again."
                attempt += 1
            except Exception as ex:
                log(f"⚠️ Error for '{full_name}' on attempt {attempt}: {ex}")
                last_server_msg = str(ex)
                attempt += 1
